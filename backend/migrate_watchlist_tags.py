...
def migrate_watchlist():
    ...
    for user_id, old_value in users:
        ...
        if old_list and isinstance(old_list[0], dict) and 'code' in old_list[0]:
            logger.info(f"用户 {user_id}: 已是新格式，跳过")
            continue
        ...
        migrated_count += 1
        preset_init_count += 1
        logger.info(f"用户 {user_id}: 迁移完成 ({len(old_list)} 条 -> {len(new_list)} 条)")
...
def main():
    logger.info("=" * 50)
    logger.info("观察仓标签功能数据迁移")
    logger.info("=" * 50)
    
    # 检查数据库文件是否存在
    if not os.path.exists(USER_DB):
        logger.error(f"数据库文件不存在: {USER_DB}")
        return
    
    logger.info(f"数据库: {USER_DB}")
    
    # 1. 创建表
    logger.info("[1/3] 创建 user_tags 表...")
    create_user_tags_table()
    
    # 2. 迁移 watchlist
    logger.info("[2/3] 迁移 watchlist 数据格式...")
    wl_count, preset_count = migrate_watchlist()
    logger.info(f"watchlist 迁移完成: {wl_count} 个用户")
    logger.info(f"预设标签初始化完成: {preset_count} 个用户")
    
    # 3. 迁移 portfolio（可选）
    logger.info("[3/3] 迁移 portfolio 数据格式...")
    pf_count = migrate_portfolio()
    logger.info(f"portfolio 迁移完成: {pf_count} 个用户")
    ...
    logger.info("2. 测试标签功能")
    logger.info("3. 如需回滚，删除 user_tags 表即可")
